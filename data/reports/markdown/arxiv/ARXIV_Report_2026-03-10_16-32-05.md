# 📊 ArXiv 研究报告 (2026-03-10)

> 生成时间: 2026-03-10 16:32:05
> 数据源: ArXiv

## 📌 配置信息

### 关键词列表（共 3 个，总权重 3.0）

| 关键词 | 权重 | 类型 |
|--------|------|------|
| medical image analysis | 1.0 | 主要 |
| medical imaging | 1.0 | 主要 |
| deep learning medical imaging | 1.0 | 主要 |

### 评分设置

- **每个关键词最大分**: 10
- **及格分公式**: 5.0 + 3.0 × 总权重
- **当前及格分**: 14.0

## 📈 论文统计

- **总抓取**: 28 篇
- **及格论文**: 0 篇 (0.0%)

---

## 📋 所有论文列表

### 1. ❌ Coupled-Layer Construction of Quantum Product Codes

**作者**: Shuyu Zhang, Tzu-Chieh Wei, Nathanan Tantivasadakarn
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08711v1](http://arxiv.org/abs/2603.08711v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为《Coupled-Layer Construction of Quantum Product Codes》，摘要内容完全围绕量子纠错码、产品码、量子低密度奇偶校验码、张量积码、平衡积码、耦合层构造、任意子凝聚等量子计算和拓扑相领域的概念展开。全文未提及任何医学图像分析、医学成像或深度学习医学成像相关内容，与给定的三个关键词完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了如何通过耦合层构造方法直观地构建量子产品码，解决了从组成码组装一般产品码的物理机制不明确的问题，并统一了通过任意子凝聚构造高维拓扑相的已知物理机制。

<details open>
<summary>摘要翻译</summary>

> 乘积码是一类由两个或多个组成码构建的量子纠错码。近期，因其在量子低密度奇偶校验（qLDPC）码领域取得突破性进展，使得码距与编码率均能实现有利的缩放关系，此类码备受关注。然而，尽管其代数表述形式强大，从组成码组装出一般乘积码的物理机制仍不明确。在本文中，我们证明张量积码与平衡积码可通过一种直观的耦合层构造实现：取一种码的多层堆叠，并按照另一种码的校验模式凝聚一组激发。我们的框架兼容经典或量子CSS输入码，统一了通过任意子凝聚构建高维拓扑相的所有已知物理机制，并能自然地推广至非拓扑码。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Product codes are a class of quantum error correcting codes built from two or more constituent codes. They have recently gained prominence for a breakthrough yielding quantum low-density parity-check (qLDPC) codes with favorable scaling of both code distance and encoding rate. However, despite its powerful algebraic formulation, the physical mechanism for assembling a general product code from its constituents remains unclear. In this letter, we show that the tensor and balanced product codes admit an intuitive coupled-layer construction by taking a stack of one code and condensing a set of excitations in the pattern given by the checks of the other code. Our framework accommodates both classical or quantum CSS input codes, unifies known physical mechanisms for constructing higher dimensional topological phases via anyon condensation, and naturally extends to non-topological codes.

</details>
<br>

**关键词**: quantum error correcting codes, product codes, quantum LDPC codes, coupled-layer construction, tensor product codes, balanced product codes, anyon condensation, topological phases

---

### 2. ❌ Approximate QCAs in one dimension using approximate algebras

**作者**: Daniel Ranard, Michael Walter, Freek Witteveen
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08702v1](http://arxiv.org/abs/2603.08702v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究量子细胞自动机（QCAs）的近似理论，属于量子计算和数学物理领域，与医学图像分析、医学成像、深度学习医学成像等关键词完全无关。论文内容涉及量子系统、代数结构、局部性条件等理论物理概念，没有任何医学或图像处理相关内容。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了在一维有限圆上的近似量子细胞自动机，证明了它们可以通过局部边界代数构造被精确化为严格量子细胞自动机，从而保持了与精确情况相同的分类指标。

<details open>
<summary>摘要翻译</summary>

> 量子细胞自动机（Quantum Cellular Automata，简称QCAs）是张量积代数的自同构映射，其保持定域性，局域量子电路即为一简单示例。本文研究近似量子细胞自动机，其中定域性条件仅在小误差范围内满足，正如晶格上的局域量子动力学所呈现的情形。从理论上讲，近似量子细胞自动机可能展现出本质上全新的行为，无法被任何精确量子细胞自动机良好逼近。我们证明在一维情形下这一现象不会发生：有限圆环上的每一个近似量子细胞自动机均可被修正为一个严格的量子细胞自动机，且其对局域算子的作用大致相同，因此这些系统可通过与精确情形相同的指标进行分类。先前的研究通过使用不适用于有限系统的全局方法，处理了无限直线上的情形。我们的新方法采用局域化处理，现可应用于有限系统，包括圆环或从子区间出发的同态映射。我们从限制在局域区块上的近似量子细胞自动机中提取出精确的局域边界代数，进而将这些代数拼接以构成严格的量子细胞自动机。关键技术要素在于两个子代数交集的鲁棒性概念：当两个子代数上的投影近似对易时，我们构造出一个精确子代数，作为其交集的一个稳定代理。该构造利用了基塔耶夫（Kitaev）近期关于近似$C^*$-代数刚性的定理。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Quantum cellular automata (QCAs) are automorphisms of tensor product algebras that preserve locality, with local quantum circuits as a simple example. We study approximate QCAs, where the locality condition is only satisfied up to a small error, as occurs for local quantum dynamics on the lattice. A priori, approximate QCAs could exhibit genuinely new behavior, failing to be well-approximated by any exact QCA. We show this does not occur in one dimension: every approximate QCA on a finite circle can be rounded to a strict QCA with approximately the same action on local operators, so these systems are classified by the same index as in the exact case. Previous work considered the case of the infinite line, by using global methods not amenable to finite systems. Our new approach proceeds locally and now applies to finite systems, including circles or homomorphisms from sub-intervals. We extract exact local boundary algebras from the approximate QCA restricted to local patches, then glue these to form a strict QCA. The key technical ingredient is a robust notion of the intersection of two subalgebras: when the projections onto two subalgebras approximately commute, we construct an exact subalgebra that serves as a stable proxy for their intersection. This construction uses a recent theorem of Kitaev on the rigidity of approximate $C^*$-algebras.

</details>
<br>

**关键词**: quantum cellular automata, approximate QCAs, one-dimensional systems, local boundary algebras, C*-algebras, quantum circuits, finite systems, index classification

---

### 3. ❌ Four negations and the spectral presheaf

**作者**: Benjamin Engel, Ryshard-Pavel Kostecki
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08699v1](http://arxiv.org/abs/2603.08699v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为'Four negations and the spectral presheaf'，摘要内容完全围绕数理逻辑、代数结构（如(co)quasiintuitionistic algebras、Akchurin algebras）、谱预层（spectral presheaf）的推广、量子力学的拓扑理论方法以及否定运算符的数学理论研究。全文未提及任何医学图像分析、医学成像或深度学习在医学成像中的应用，与给定的三个医学领域关键词完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了如何将谱预层框架推广到任意完备正交补格，证明了正交补决定了谱预层上的两个否定运算符，并构建了具有四个不同否定的双拟直觉主义代数和Akchurin代数模型，同时证明了谱预层不能作为相关逻辑的模型。

<details open>
<summary>摘要翻译</summary>

> 基于瓦卡雷洛夫（Vakarelov）的带否定格逻辑理论，我们引入了（共）准直觉主义逻辑，并证明了其相对于（共）准直觉主义代数类的可靠性与完备性。将这些代数结合，我们得到了双准直觉主义代数及相应的逻辑。进一步通过斯科伦（Skolem）代数结构扩展它们，定义了阿克丘林（Akchurin）代数及其对应逻辑。接着，我们将谱预层（spectral presheaf）的框架（该框架是Butterfield–Isham–Döring量子力学的拓扑斯理论方法中的核心对象）推广至任意完备正交补格，并证明正交补决定了谱预层上的两个否定算子（一个为次协调否定，另一个为次完全否定），从而赋予谱预层所有闭开子预层集合以双准直觉主义代数结构。结合该集合上一般的斯科伦（即海廷与布劳威尔）代数结构，我们得到了阿克丘林代数的一个具体实例，它是双准直觉主义逻辑与双直觉主义逻辑乘积的可靠模型，具有四种不同的否定算子。我们还证明，基础的正交补格可作为谱预层的内蕴对象被重构，表现为双重共准直觉主义（相应地，准直觉主义）否定单子（相应地，余单子）的像。最后，我们针对“谱预层是辩证（或任何其他）相干逻辑模型”的论断证明了一个不可行定理。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Using Vakarelov's theory of lattice logics with negation, we introduce the (co)quasiintuitionistic logic, and prove its soundness and completeness with respect to the class of (co)quasiintuitionistic algebras. Combining these algebras together, we obtain biquasiintuitionistic algebras and the corresponding logic. Their further extension with the Skolem algebra structure defines Akchurin algebras and the respective logic. Next we generalise the framework of spectral presheaves (which is a main object in the Butterfield--Isham--Döring topos theoretic approach to quantum mechanics) to arbitrary complete orthocomplemented lattices, and show that the orthocomplementation determines two negation operators on the spectral presheaf (one paraconsistent, another paracomplete), equipping the set of all closed-and-open subpresheaves of a spectral presheaf with the structure of a biquasiintuitionistic algebra. Combined with the generic Skolem (i.e. Heyting and Brouwer) algebra structure of this set, this gives a particular instance of an Akchurin algebra, which is a sound model of a product of biquasiintuitionistic and biintuitionistic logics, featuring four distinct negations. We also show that the underlying orthocomplemented lattice can be reconstructed as an internal object of the spectral presheaf, resulting as the image of a double coquasiintuitionistic (resp., quasiintuitionistic) negation monad (resp., comonad). Finally, we prove a no-go theorem for the claim that the spectral presheaf is a model of a dialectical (or any other) relevance logic.

</details>
<br>

**关键词**: spectral presheaf, biquasiintuitionistic algebra, Akchurin algebra, negation operators, orthocomplemented lattices, quantum mechanics, topos theory, no-go theorem

---

### 4. ❌ Scalable Postselection of Quantum Resources

**作者**: J. Wilson Staples, Winston Fu, Jeff D. Thompson
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08697v1](http://arxiv.org/abs/2603.08697v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为《Scalable Postselection of Quantum Resources》，摘要内容完全围绕量子计算、量子纠错码、后选择技术、逻辑错误率等量子计算领域的概念展开。论文未提及任何医学图像分析、医学成像或深度学习在医学成像中的应用，与给定的三个关键词完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究如何通过可扩展的后选择技术降低量子计算的资源开销，并证明该方法能在相同逻辑错误概率下将每个逻辑门的开销减少4倍。

<details open>
<summary>摘要翻译</summary>

> 量子纠错带来的巨大开销是实现量子计算机的关键挑战，这促使人们寻找替代的纠错码和容错电路构造方案。后选择是一种强大的工具，它通过概率生成的子电路构建大型程序，并已被证明能够提升基于融合固定尺寸资源态或级联码的量子纠错阈值。在本工作中，我们提出了一种利用可扩展后选择来降低量子计算开销的方法，该方法基于使用解码器软信息直接对尺寸与码距成比例的子电路进行后选择。我们引入了一个称为“部分间隙”的度量，用于估计资源态在被消耗后其逻辑间隙的预期值，并证明基于部分间隙的后选择能够带来逻辑错误率的可扩展性改善。在通过簇态隐形传态实现逻辑门的具体背景下，我们证明了在相同逻辑错误概率下，可扩展后选择能使每个逻辑门的开销降低至原来的四分之一。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> The large overhead imposed by quantum error correction is a critical challenge to the realization of quantum computers, and motivates searching for alternative error correcting codes and fault-tolerant circuit constructions. Postselection is a powerful tool that builds large programs out of probabilistically generated sub-circuits, and has been shown to increase the threshold of quantum error correction based on fusing fixed-size resource states or concatenated codes. In this work, we present an approach to lower the overhead of quantum computing using scalable postselection, based on directly postselecting sub-circuits with a size extensive in the code distance using decoder soft information. We introduce a metric, the partial gap, that estimates what the logical gap of a resource state will be after it is consumed, and show that postselection based on the partial gap leads to scalable improvements in the logical error rate. In the specific context of implementing logical gates via teleportation through a cluster state, we demonstrate that scalable postselection provides a $4\times$ reduction in the overhead per logical gate, at the same logical error probability.

</details>
<br>

**关键词**: quantum computing, quantum error correction, postselection, logical error rate, scalable postselection, resource states, overhead reduction, cluster state

---

### 5. ❌ A note on large-scale quantum chemistry on quantum computers: the case of a molecule with half-Möbius topology

**作者**: Samuele Piccinelli, Stefano Barison, Alberto Baiardi, Francesco Tacchino, Jascha Repp, Igor Rončević, Florian Albrecht, Harry L. Anderson, Leo Gross, Alessandro Curioni, Ivano Tavernelli
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08696v1](http://arxiv.org/abs/2603.08696v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题和摘要明确表明该研究是关于量子化学计算和量子计算在分子电子结构模拟中的应用，具体涉及半莫比乌斯拓扑分子、量子处理器、SqDRIFT算法等。全文未提及任何医学图像分析、医学成像或深度学习在医学成像中的应用，与所有评分关键词完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该研究利用超导量子处理器和SqDRIFT算法对具有半莫比乌斯电子拓扑的分子进行量子化学计算，成功将模拟规模扩展到100量子比特，为量子辅助电子结构计算提供了可行路径。

<details open>
<summary>摘要翻译</summary>

> 我们报告了在超导量子处理器上对具有Rončević等人最初提出的半莫比乌斯电子拓扑结构的分子进行的量子化学计算。通过采用SqDRIFT——一种基于随机采样的Krylov量子对角化算法，我们在对应于36个轨道（72个量子比特）的活性空间上实现了可靠的量子模拟，并将先前的研究扩展至50个轨道（100个量子比特）。我们证明，系统性地增加活性空间尺寸——这对电子结构描述的准确性具有实质性影响——是可以通过当前最先进的量子处理器实现的，从而为走向具有实际意义的量子辅助电子结构计算提供了一条前景广阔的路径。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> We report quantum chemistry calculations performed on superconducting quantum processors for a molecule exhibiting the half-Möbius electronic topology originally introduced by Rončević et al. Using SqDRIFT, a randomized sample-based Krylov quantum diagonalization algorithm, we achieve reliable quantum simulations on active spaces corresponding to 36 orbitals (72 qubits) and extend previous studies up to 50 orbitals (100 qubits). We demonstrate that a systematic increase of active space sizes, which has a concrete impact on the accuracy of the electronic structure description, is achievable with state-of-the-art quantum processors, thus offering a promising path towards practically relevant quantum-assisted electronic-structure calculations.

</details>
<br>

**关键词**: quantum chemistry, quantum computers, superconducting quantum processors, SqDRIFT algorithm, half-Möbius topology, electronic structure, Krylov quantum diagonalization, active spaces

---

### 6. ❌ Fermi-pressure-assisted cavity superradiance in a mesoscopic Fermi gas

**作者**: Francesca Orsi, Ekaterina Fedotova, Rohit Prasad Bhatt, Mae Eichenberger, Léa Dubois, Jean-Philippe Brantut
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08691v1](http://arxiv.org/abs/2603.08691v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究的是介观费米气体在光学腔中的超辐射相变，属于原子物理、量子光学和凝聚态物理领域。论文内容完全不涉及医学图像分析、医学成像或深度学习医学成像，没有医学图像数据、医学应用或相关技术方法。所有关键词与论文主题完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该研究探索了介观费米气体在光学腔中的超辐射相变，发现超辐射阈值随密度呈非单调变化，并在费米波矢与反冲波矢相当时达到最小值，揭示了费米压力辅助有序与泡利阻塞光子散射之间的交叉行为。

<details open>
<summary>摘要翻译</summary>

> 我们研究了由数十至数千个$^6$Li原子组成的介观费米气体在高精细度光学腔中跨越宽密度范围内的超辐射相变。我们观察到超辐射阈值随密度呈现非单调变化，当费米波矢与反冲波矢相当时达到最小值。该最小值对应着费米压力辅助有序化与光子散射的泡利阻塞之间的交叉转变，与理论预测高度吻合。这一解释通过对有序化阈值和光子数标度的原子数依赖性研究得到进一步验证。最后，我们展示了该介观系统在一种特殊机制下的运行状态：光诱导力对两种自旋分量方向相反，从而形成具有自旋密度波特性的有序相。我们的系统为研究具有强相干光-物质耦合的少费米子体系开辟了新前景。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> We study the superradiant phase transition of a mesoscopic Fermi gas comprising between a few tens and a few thousand $^6$Li atoms in a high-finesse cavity across a wide range of densities. We observe a non-monotonic variation of the superradiant threshold as a function of density, with a minimum reached when the Fermi and recoil wavevectors are comparable. The minimum corresponds to a crossover between Fermi pressure-assisted ordering and Pauli blocking of photon scattering, in good agreement with theory. This interpretation is confirmed by a study of the atom-number dependence of the ordering threshold and photon number scaling. Lastly, we demonstrate the operation of our mesoscopic system in a regime where light-induced forces are opposite for the two spin components, leading to an ordered phase with a spin-density-wave character. Our system opens the perspective of studying few-fermion systems with strong and coherent light-matter coupling.

</details>
<br>

**关键词**: mesoscopic Fermi gas, superradiant phase transition, optical cavity, Fermi pressure, Pauli blocking, spin-density-wave, light-matter coupling, few-fermion systems

---

### 7. ❌ Metriq: A Collaborative Platform for Benchmarking Quantum Computers

**作者**: Alessandro Cosentino, Changhao Li, Vincent Russo, Bradley A. Chase, Tom Lubinski, Siyuan Niu, Neer Patel, Nathan Shammah, William J. Zeng
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08680v1](http://arxiv.org/abs/2603.08680v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文《Metriq: A Collaborative Platform for Benchmarking Quantum Computers》专注于量子计算机的基准测试平台开发，涉及量子计算、基准测试、硬件评估等领域。论文内容完全不涉及医学图像分析（medical image analysis）、医学成像（medical imaging）或深度学习医学成像（deep learning medical imaging），因此所有关键词的相关度均为0分。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文针对量子计算机基准测试碎片化的问题，开发了Metriq平台，实现了可重复的跨平台量子基准测试，并收集了多个硬件供应商的数据进行系统比较。

<details open>
<summary>摘要翻译</summary>

> 量子计算机基准测试领域呈现碎片化格局，其特点是系统专用工具与不一致的评估方法并存，这阻碍了可靠的跨平台性能评估。我们推出Metriq——一个用于可复现跨平台量子基准测试的开源协作平台，它将基准测试的定义与执行、数据收集和公开呈现整合至统一工作流中。Metriq基准测试套件既包含表征纠缠质量、门性能及电路速度等基础设备特性的系统级指标，也涵盖评估量子机器学习、优化和量子模拟任务性能的应用导向型协议。所选基准测试可随处理器规模扩展，且该框架整合了成本与资源估算以支持实际评估。利用Metriq，我们收集并公开发布了来自多家硬件供应商的十余台量子计算机的测试结果，实现了系统化的跨平台比较。由此产生的精选数据集还揭示了各基准测试的实际优势与局限，形成了推动测试套件持续优化的反馈闭环。为综合评估整套基准测试的性能，我们引入了Metriq分数——一个聚合基准测试结果的复合指标。基于共享数据集，我们进一步展示了跨基准测试分析及其与硬件校准指标的相关性。通过开放开发和数据共享，Metriq为量子计算机的可复现基准测试提供了实践基础，这将持续推动硬件与基准测试方法的协同演进。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> The fragmented landscape of quantum computer benchmarks, characterized by system-specific tools and inconsistent evaluation methodologies, hinders reliable cross-platform performance assessment. We introduce Metriq, an open-source collaborative platform for reproducible cross-platform quantum benchmarking that integrates benchmark definition and execution, data collection, and public presentation into a unified workflow. The Metriq benchmark suite spans both system-level metrics that characterize fundamental device properties such as entanglement quality, gate performance, and circuit speed, as well as application-inspired protocols that assess performance on quantum machine learning, optimization, and quantum simulation tasks. Benchmarks are chosen to scale with processor size, and the framework incorporates cost and resource estimation to support practical evaluation. Using Metriq, we collect and publicly release results from more than ten quantum computers across multiple hardware vendors, enabling systematic cross-platform comparison. The resulting curated dataset also reveals the practical strengths and limitations of individual benchmarks, creating a feedback loop that informs the ongoing refinement of the suite. To summarize performance across the benchmark suite, we introduce the Metriq Score, a composite index aggregating benchmark outcomes. We further present cross-benchmark analyses enabled by the shared dataset and their correlations with hardware calibration metrics. Through open development and data sharing, Metriq provides a practical foundation for reproducible benchmarking of quantum computers as hardware and benchmarking methods continue to evolve.

</details>
<br>

**关键词**: quantum computing, benchmarking, cross-platform comparison, reproducible evaluation, quantum machine learning, hardware performance, open-source platform, Metriq Score

---

### 8. ❌ Characterization and upgrade of a quantum graph neural network for charged particle tracking

**作者**: Matteo Argenton, Laura Cappelli, Concezio Bozzi
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08667v1](http://arxiv.org/abs/2603.08667v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究量子图神经网络在高能物理粒子追踪中的应用，与医学图像分析完全无关。论文内容涉及量子机器学习、高能物理、粒子追踪、图神经网络等，未提及任何医学图像、医学成像或相关应用。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了量子图神经网络在模拟高亮度数据集上重建带电粒子轨迹的性能，通过升级混合架构改进了训练收敛行为。

<details open>
<summary>摘要翻译</summary>

> 未来几年，LHC实验将进行升级以利用其瞬时亮度的显著提升，这将导致更大规模、更高密度的事件，进而增加带电粒子径迹重建的复杂性，从而推动新技术的尖端研究。量子机器学习模型正被探索作为高能物理（HEP）任务的潜在新方法。我们针对模拟高亮度数据集中的带电粒子径迹重建任务，对一种量子图神经网络（QGNN）架构进行了特性分析与升级。该模型基于一组事件图运行，每个事件图由质子对撞产生的粒子在径迹探测器各层中生成的击中点构建，并对相邻层之间可能的击中点连接进行分类。在此方法中，QGNN被设计为混合架构，将经典前馈网络与参数化量子电路交错结合。我们分析了经典组件与量子组件之间的相互作用，报告了对原始设计的主要升级，并提供了训练行为改善的新证据，特别是在向最终训练配置收敛方面。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> In the forthcoming years the LHC experiments are going to be upgraded to benefit from the substantial increase of the LHC instantaneous luminosity, which will lead to larger, denser events, and, consequently, greater complexity in reconstructing charged particle tracks, motivating frontier research in new technologies. Quantum machine learning models are being investigated as potential new approaches to high energy physics (HEP) tasks. We characterize and upgrade a quantum graph neural network (QGNN) architecture for charged particle track reconstruction on a simulated high luminosity dataset. The model operates on a set of event graphs, each built from the hits generated in tracking detector layers by particles produced in proton collisions, performing a classification of the possible hit connections between adjacent layers. In this approach the QGNN is designed as a hybrid architecture, interleaving classical feedforward networks with parametrized quantum circuits. We characterize the interplay between the classical and quantum components. We report on the principal upgrades to the original design, and present new evidence of improved training behavior, specifically in terms of convergence toward the final trained configuration.

</details>
<br>

**关键词**: quantum graph neural network, charged particle tracking, high energy physics, track reconstruction, hybrid architecture, quantum machine learning, LHC experiments

---

### 9. ❌ Symmetry-based perturbation theory for electronic structure calculations

**作者**: Hiromichi Nishimura, Nam Nguyen, Tanvi Gujarati, Mario Motta
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08631v1](http://arxiv.org/abs/2603.08631v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题和摘要明确表明这是一篇关于电子结构计算的理论物理/计算化学论文，涉及对称性微扰理论、哈密顿量、量子计算等概念。所有评分关键词（medical image analysis, medical imaging, deep learning medical imaging）均与医学图像分析相关，而论文内容完全不涉及医学、图像处理或深度学习在医学领域的应用。因此所有关键词相关度均为0分。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文提出了一种基于哈密顿量对称性的多参考微扰理论（SBPT），用于电子结构计算，能够减少计算资源配置和量子比特需求，并在部分分子体系中表现出更好的稳健性。

<details open>
<summary>摘要翻译</summary>

> 我们基于哈密顿量的对称性，发展了一种用于电子结构计算的多参考微扰理论。在对称性微扰理论中，参考哈密顿量的选择使其比原始哈密顿量具有更多对称性，从而在组态相互作用展开中的组态数量以及量子计算应用中所需的量子比特数方面，均能更大程度地减少计算资源消耗。我们为二阶修正提供了近似且可扩展的解决方案，并将其应用于选定组态相互作用。我们证明，对称性微扰理论是其他现有多参考微扰理论的扩展，并且能够以稳健的方式为某些分子体系提供更好的计算结果。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> We develop a multi-reference perturbation theory for electronic structure calculations based on symmetries of the Hamiltonian. The reference Hamiltonian in the symmetry-based perturbation theory (SBPT) is chosen such that it possesses more symmetries than the original Hamiltonian, leading to a larger reduction of computational resources in terms of both the number of configurations in the configuration interaction expansion and the number of required qubits in quantum computing applications. We provide approximate, scalable solutions for the second-order correction, as well as an application to selected configuration interaction. We show that SBPT is an extension of other existing multi-reference perturbation theories and that it can give better results for some molecular systems in a robust way.

</details>
<br>

**关键词**: symmetry-based perturbation theory, electronic structure calculations, Hamiltonian symmetries, quantum computing, configuration interaction, multi-reference perturbation theory, computational resources reduction

---

### 10. ❌ Bias in Local Spin Measurements from Deformed Symmetries

**作者**: Michele Arzano, Goffredo Chirco, Jerzy Kowalski-Glikman
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08618v1](http://arxiv.org/abs/2603.08618v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究量子群对称性下的自旋测量偏差问题，属于理论物理和量子信息领域，与医学图像分析、医学成像、深度学习医学成像等关键词完全无关。论文内容涉及量子群、对称性、自旋关联、贝尔态、局部测量等概念，没有任何医学图像或医学应用的内容。

</details>
<br>

!!! tip deepseek-chat TL;DR

    论文研究了在量子群对称性框架下，局部自旋测量如何因对称性变形而产生偏差，并提出了使用R矩阵修饰的局部可观测量来恢复无偏统计的方法。

<details open>
<summary>摘要翻译</summary>

> 我们研究当旋转对称性由量子群而非普通李群描述时的二分量自旋单态关联。尽管单自旋可观测量在形式上与未变形理论一致，但非平凡余积重塑了“整体”对称性的概念，并导出了贝尔单态的一种变形类比。我们发现，采用传统张量积因子可观测量进行局域测量时会产生显著效应：尽管完美反关联性得以保持，但单点测量结果的统计分布会依赖于变形参数并产生偏差。若改用具有对称协变性、经R矩阵修饰的局域可观测量嵌入方式，则可在维持完美反关联性的同时恢复无偏统计分布。我们的研究结果表明，在量子群对称性框架下，严格的张量积因子局域性在对称变换下并不稳定，必须代之以辫子化局域性概念才能构建自洽的局域测量理论。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> We study bipartite spin-singlet correlations when rotational symmetry is described by a quantum group rather than an ordinary Lie group. We show that, even though the single-spin observables act as in the undeformed theory, the non-trivial coproduct reshapes the notion of "total" symmetry and leads to a deformed analogue of the Bell singlet state. We show that implementing local measurements with the conventional tensor-factor observables yields a striking effect: perfect anticorrelation is preserved, yet the one-site outcome statistics become deformation-dependent and biased. Using instead the symmetry-covariant, R-matrix-dressed embedding of local observables restores unbiased statistics while maintaining perfect anticorrelation. Our results demonstrate that, in a quantum group symmetry setting, strict tensor-factor locality is not stable under the symmetry and must be replaced by a braided notion of locality to formulate consistent local measurements.

</details>
<br>

**关键词**: quantum group symmetry, spin-singlet correlations, Bell singlet state, local measurements, deformed symmetries, R-matrix, bipartite correlations, braided locality

---

### 11. ❌ Communication-constrained nonlocal correlations

**作者**: Lucas Pollyceno, Denis Freudenheim, José Nogueira, Anubhav Chaturvedi, Rafael Rabelo, Marcin Pawłowski
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08610v1](http://arxiv.org/abs/2603.08610v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为'Communication-constrained nonlocal correlations'，摘要讨论量子理论与更广泛概率框架的区别，聚焦于通信约束、信息因果性、通信任务和信息论框架。全文未提及医学图像分析、医学成像或深度学习医学成像相关内容，研究领域属于量子信息理论、基础物理和通信理论，与医学图像分析完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该研究通过系统扩展基于通信的方法，识别相关通信任务类别，并利用信息论框架推导新的操作约束，揭示了一类先前未检测到的不可信行为，强化了通信作为识别物理有意义理论的基本视角。

<details open>
<summary>摘要翻译</summary>

> 区分量子理论与更广泛概率框架的物理基础仍是一个悬而未决的挑战。基于通信的研究方案——尤其是超光速信号传递的不可能性原理与信息因果性（IC）原理——强调了通信在排除非物理理论中的作用，并为量子预测为何优于其他替代模型提供了操作性的理论依据。然而，此类研究大多依赖于在特定任务（如通信复杂度问题与随机存取编码）中对通信环节的优化。本文系统性地拓展了这一基于通信的研究路径。我们界定了与此背景相关的通信任务类别，并运用广义信息论框架推导出新的操作性约束，以阻止此类非物理行为。值得注意的是，我们的研究结果揭示了一大类先前未被发现的非合理行为，这些行为独立于任何特定的编码或解码策略，从而强化了通信作为识别具有物理意义理论的基本视角的作用。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Identifying the physical grounds distinguishing quantum theory from broader probabilistic frameworks remains an open challenge. Communication-based proposals -- most notably the principles of impossibility of superluminal signaling and information causality (IC) -- highlight the role of communication in ruling out unphysical theories and offer an operational rationale on why quantum predictions prevail over these alternative models. Nevertheless, most such developments rely on communicating parts optimizing over specific tasks, such as communication complexity problems and random access codes (RAC). In this work, we systematically extend this communication-based approach. We characterize the class of communication tasks relevant for this context, and employ the general information-theoretic framework to derive new operational constraints preventing such unphysical behaviors. Remarkably, our results reveal a broad family of previously undetected implausible behaviors, independent of any particular encoding or decoding strategy, reinforcing the role of communication as a fundamental lens through which physically meaningful theories can be identified.

</details>
<br>

**关键词**: quantum theory, communication constraints, information causality, nonlocal correlations, information-theoretic framework, operational constraints, physical theories, communication tasks

---

### 12. ❌ Heterogeneously Integrated Diamond-on-Lithium Niobate Quantum Photonic Platform

**作者**: Sophie W. Ding, Chang Jin, Zixi Li, Nicholas Achuthan, Kazuhiro Kuruma, Xinghan Guo, Brandon Grinkemeyer, David D. Awschalom, Nazar Delegan, F. Joseph Heremans, Alexander A. High, Marko Loncar
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08609v1](http://arxiv.org/abs/2603.08609v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究的是量子光子平台，涉及金刚石薄膜与铌酸锂薄膜的异质集成、光子晶体腔、硅空位光子发射等量子光学和集成光子学内容，完全不涉及医学图像分析、医学成像或深度学习医学成像。论文中未提及任何医学应用、诊断、治疗或医学图像处理技术。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文解决了金刚石单独无法提供非线性电光功能的问题，通过异质集成铌酸锂薄膜与金刚石薄膜，实现了高效的光子传输和硅空位光子收集，为量子网络提供了可扩展的集成光子电路平台。

<details open>
<summary>摘要翻译</summary>

> 金刚石光子学已为量子存储器提供了高效接口，并被预测为量子网络的关键组成部分。然而，可扩展的网络架构需要对光子进行空间、时间及光谱控制，这依赖于非线性与电光功能，而单纯的金刚石无法实现这些功能。本文展示了具有强χ²非线性和电光效应的薄膜铌酸锂平台与金刚石薄膜的异质集成。我们实现了高Q值金刚石光子晶体腔（在735纳米波长处Q因子超过5×10⁴），这些腔体通过光刻技术与薄膜铌酸锂光子主干对准并与之临界耦合。这使得我们能够实现低损耗的金刚石-薄膜铌酸锂“传输通道”（损耗约1 dB/耦合器），支持两者间的高效光传输。在低温（5K）条件下，我们可以通过薄膜铌酸锂光子电路收集嵌入金刚石结构中的硅空位色心所发射的光子。这一方法为面向实用量子网络及其他技术的集成光子电路开辟了一条可扩展的路径。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Diamond photonics has enabled efficient interfaces for quantum memories and is predicted to be a critical component of quantum networks. However, scalable network architectures require spatial, temporal, and spectral control of photons, which relies on nonlinear and electro-optic functionalities that diamond alone cannot provide. Here, we demonstrate heterogeneous integration of a thin-film lithium niobate (TFLN) platform, which has strong chi-2 nonlinearity and electro-optic effects, with thin diamond films. We demonstrate high-Q diamond photonic crystal cavities (Q factors exceeding 5x10^4 at 735 nm) that are lithographically aligned with TFLN photonic backbone and critically coupled to it. This allows us to realize low-loss diamond-TFLN "escalators" (loss ~1 dB/coupler) that support efficient light transfer between them. At cryogenic temperatures (5K), we can collect photons emitted from silicon vacancies (SiVs) embedded within the diamond structure via the TFLN photonic circuit. This approach establishes a scalable route toward integrated photonic circuits for practical quantum networking and other technologies.

</details>
<br>

**关键词**: diamond photonics, lithium niobate, heterogeneous integration, quantum photonic platform, silicon vacancies, photonic crystal cavities, quantum networking, integrated photonic circuits

---

### 13. ❌ Quantum-limited estimation of the difference between photonic momenta via spatially resolved two-photon interference

**作者**: Luca Maggio, Vincenzo Tamma
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08602v1](http://arxiv.org/abs/2603.08602v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究的是量子传感协议，利用空间分辨双光子干涉进行三维动量差估计，属于量子物理、量子光学和量子传感领域。论文内容完全不涉及医学图像分析、医学成像或深度学习医学成像，没有医学应用背景或医学数据，因此所有关键词相关度均为0分。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文提出了一种基于空间分辨双光子干涉的量子传感协议，用于三维估计两个光子的动量差，实现了量子极限精度，在约2000次采样测量中参数估计偏差低于1%。

<details open>
<summary>摘要翻译</summary>

> 本文提出了一种基于空间分辨干涉采样测量的三维光子动量差估计量子传感方案。该方案在同步估计相对动量各分量时，无论参数取值如何，仅需约2000次采样测量即可达到极限量子精度，且偏差低于1%。这些结果表明，三维空间分辨双光子干涉可作为多参数量子传感的有效工具，在自由空间量子技术的高精度三维定位、折射率测量及近场校准等领域具有潜在应用前景。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> We present a quantum sensing protocol for three-dimensional estimation of the difference between the momenta of two photons based on spatially resolved interferometric sampling measurements. The protocol attains ultimate quantum precision in the simultaneous estimation of the components of the relative momentum for any values of the parameters already with $\sim 2000$ sampling measurements and a bias below $1\%$. These results identify 3D spatially resolved two-photon interference as an efficient tool for multi-parameter quantum sensing, with potential applications in high-precision 3D localization, refractometry, and near-field calibration in free-space quantum technologies.

</details>
<br>

**关键词**: quantum sensing, two-photon interference, momentum estimation, spatially resolved, 3D localization, quantum precision, multi-parameter estimation, photonic momenta

---

### 14. ❌ Parity-dependent Casimir forces and Hall currents for a confined Dirac field

**作者**: Aitor Fernández, César D. Fosco
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08601v1](http://arxiv.org/abs/2603.08601v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究的是量子场论中的狄拉克场在受限条件下的卡西米尔效应、电流关联和霍尔电流，属于理论物理领域，与医学图像分析、医学成像、深度学习医学成像等关键词完全无关。论文内容涉及边界条件、对称性、真空涨落等物理概念，没有任何医学或图像处理相关内容。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了受限狄拉克场在不同边界条件下的卡西米尔效应，发现对称配置产生吸引力而反对称配置产生排斥力，并在2+1维度中展示了诱导的霍尔电流现象。

<details open>
<summary>摘要翻译</summary>

> 我们在d+1维空间中研究了一个无质量狄拉克场在两条平行薄壁上分别施加两种不同边界条件的情况。这两种构型对应于系统在两壁中间平面反射变换下呈现偶对称或奇对称性质，并导致本质不同的物理行为。偶对称（对称）构型产生吸引的卡西米尔力，而奇对称（反对称）构型则产生排斥力，这与连接宇称性与费米子卡西米尔效应符号的普遍定理相一致。我们通过研究与产生卡西米尔相互作用的真空涨落相关的两种现象来补充这一结论，这两种现象同样对宇称性敏感：一是集中在壁面上的电流关联，二是在外电场影响下诱导产生的体电流。针对后者我们证明，在2+1维情况下会产生诱导横向（类霍尔）电流，其空间分布形态继承了约束势场的对称性特征。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> We study a massless Dirac field subjected to two alternative boundary conditions on two parallel thin walls, in d + 1 dimensions. The two configurations correspond to the system being even or odd under reflection about the midplane between the two walls, and lead to qualitatively different behaviors. The even (symmetric) configuration produces an attractive Casimir force, whereas the odd (antisymmetric) one yields repulsion, in agreement with a general theorem linking parity to the sign of the fermionic Casimir effect. We complement this result by studying two phenomena associated with the vacuum fluctuations responsible for the Casimir interaction, both of which are also sensitive to parity: the correlation between currents concentrated on the walls, and the induced bulk current under the influence of an external electric field. For the latter we show that, in 2 + 1 dimensions, an induced transverse (Hall-like) current arises, whose spatial profile inherits the symmetry of the confining potential.

</details>
<br>

**关键词**: Dirac field, Casimir force, boundary conditions, parity symmetry, Hall current, vacuum fluctuations, quantum field theory

---

### 15. ❌ Gate Optimization via Efficient Two-Qubit Benchmarking for NV Centers in Diamond

**作者**: Alessandro Marcomini, Philipp J. Vetter, Tommaso Calarco, Felix Motzoi, Fedor Jelezko, Matthias M. Müller
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08581v1](http://arxiv.org/abs/2603.08581v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究的是量子计算领域中的氮空位中心（NV centers in diamond）的两量子比特门优化方法，通过高效的基准测试实现闭环优化。论文内容完全聚焦于量子物理、量子控制和量子信息处理，没有涉及任何医学图像分析、医学成像或深度学习在医学成像中的应用。所有关键词与论文主题完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文提出了一种通过仅测量两个量子态来高效评估两量子比特门性能的方法，实现了氮空位中心量子系统的实验闭环优化，相比标准过程层析技术将所需测量次数减少了两个数量级。

<details open>
<summary>摘要翻译</summary>

> 高保真度量子门实现需要精密的控制脉冲来引导量子系统完成预期变换。量子最优控制技术能够基于数值模拟以开环方式推导这些控制脉冲，但其精度可能受限于系统认知的不完备性。闭环优化通过引入测量反馈克服了这一局限，前提是能够定义合适且高效的量子门性能度量标准。本文提出一种仅需制备并测量两个量子态即可评估双量子比特门性能的高效方法，使得此前被认为仅适用于开环控制的度量标准得以应用于实验闭环优化。我们将该方法针对金刚石中的氮空位色心体系进行定制，并通过数值模拟证明：在现实实验条件下，相较于标准量子过程层析技术，此方法能够在优化双量子比特门的同时将所需测量次数降低两个数量级。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> High-fidelity gate implementation requires sophisticated control pulses that steer the quantum system to undergo the desired transformation. Quantum Optimal Control allows to derive these control pulses in an open-loop fashion based on numerical simulations. However, their precision can be limited by incomplete knowledge of the system. Closed-loop optimization overcomes this limitation by incorporating feedback from measurements, provided a suitable and efficient measure of the gate performance can be defined. In this article, we present an efficient method to evaluate the performance of a two-qubit gate by preparation and measurement of only two quantum states, enabling experimental closed-loop optimization with a metric previously believed to be limited to open-loop control. We tailor the approach to nitrogen-vacancy centers in diamond and, through numerical simulations, demonstrate how the method can optimize a two-qubit gate while reducing the number of required measurements by two orders of magnitude compared to standard process tomography under realistic experimental settings.

</details>
<br>

**关键词**: quantum optimal control, two-qubit gate, nitrogen-vacancy centers, closed-loop optimization, gate benchmarking, quantum measurement, diamond defects, quantum simulation

---

### 16. ❌ The Grasshopper Problem on the Sphere

**作者**: David Llamas, Dmitry Chistikov, Adrian Kent, Mike Paterson, Olga Goulko
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08579v1](http://arxiv.org/abs/2603.08579v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为'The Grasshopper Problem on the Sphere'，摘要内容完全围绕几何优化问题、贝尔不等式、量子单重态关联、球面离散化、球谐函数展开等物理和数学主题展开。全文未提及任何医学图像分析、医学成像或深度学习在医学成像中的应用，与所有评分关键词完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了球面上的蚱蜢问题，这是一个与贝尔不等式和量子单重态模拟相关的几何优化问题，通过分析球面离散化和不同变体，揭示了最优草坪配置的几何结构及其球谐函数展开表示。

<details open>
<summary>摘要翻译</summary>

> 球面蚱蜢问题是一个源于贝尔不等式背景的几何优化问题，可理解为：对于沿固定夹角随机轴方向的测量，寻找对量子单态关联的最佳局域隐变量近似。在同期发表的论文[arXiv:2504.20953]中，我们给出了该问题的数值解，并阐释了其在单态模拟与检验中的意义。在本篇配套论文中，我们将详细阐述这些结果所依托的几何与计算框架。我们探讨了球面离散化的作用，并比较了该问题的三种自然变体：对径互补草坪、对径独立草坪以及非对径互补草坪。我们分析了相应最优草坪构型的几何结构，并通过球谐函数展开进行解释。此外，我们还讨论了该问题与其他物理模型以及几何概率中经典问题的联系。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> The spherical grasshopper problem is a geometric optimization problem that arises in the context of Bell inequalities and can be interpreted as identifying the best local hidden variable approximation to quantum singlet correlations for measurements along random axes separated by a fixed angle. In a parallel publication [arXiv:2504.20953], we presented numerical solutions for this problem and explained their significance for singlet simulation and testing. In this companion paper, we describe in detail the geometric and computational framework underlying these results. We examine the role of spherical discretization and compare three natural variants of the problem: antipodal complementary lawns, antipodal independent lawns, and non-antipodal complementary lawns. We analyze the geometric structure of the corresponding optimal lawn configurations and interpret it in terms of a spherical harmonics expansion. We also discuss connections to other physical models and to classical problems in geometric probability.

</details>
<br>

**关键词**: spherical grasshopper problem, geometric optimization, Bell inequalities, quantum singlet correlations, spherical discretization, spherical harmonics, lawn configurations, geometric probability

---

### 17. ❌ Microwave response of electrically driven spins in a three-qubit quantum processor

**作者**: Tanner M. Janda, Heun Mo Yoo, Connor Nasseraddin, Adam R. Mills, Zhaoyi Joy Zheng, Jason R. Petta
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08577v1](http://arxiv.org/abs/2603.08577v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究的是量子计算领域的三量子比特处理器中电驱动自旋的微波响应，属于量子物理和量子计算范畴，与医学图像分析、医学成像、深度学习医学成像等医学图像处理领域完全无关。论文内容涉及电偶极自旋共振、量子比特、微波驱动等量子物理概念，没有任何医学图像或医学数据分析的内容。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了量子处理器中电驱动自旋的微波响应特性，发现当微波幅度经过仔细校准时，拉比频率与驱动幅度呈线性关系，且非共振驱动引起的热致共振频移与典型时间漂移相当，表明先前观察到的非线性响应并非LD自旋量子比特的普遍特征。

<details open>
<summary>摘要翻译</summary>

> 在电偶极自旋共振（EDSR）中，单个自旋在微磁体产生的磁场梯度下被电驱动。尽管EDSR已在多种器件中实现了高保真度门操作，但已有报告指出拉比频率随微波驱动幅度变化时存在意外的非线性响应。本研究对洛斯-迪文森佐（Loss-DiVincenzo, LD）单自旋量子比特在共振驱动以及同时施加共振与非共振驱动（如实际量子处理器中可能出现的情况）下的响应进行了精细测量。在精确校准微波幅度后，我们发现即使同时驱动全部三个自旋，拉比频率仍与驱动幅度呈线性比例关系。同时，我们确定非共振驱动引起的热致共振频移与典型的时间漂移量级相当。我们的结果表明，先前观测到的非线性响应并非LD自旋量子比特的普遍特性。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> In electric dipole spin resonance (EDSR), a single spin is electrically driven in the field gradient produced by a micromagnet. While EDSR has enabled high fidelity gate operations in many devices, there are reports of unexpected non-linearities in the Rabi frequency as a function of microwave drive amplitude. We carefully measure the response of Loss-DiVincenzo (LD) single spin qubits to resonant drives as well as simultaneous resonant and off-resonant drives, as would be encountered in a realistic quantum processor. With the microwave amplitude carefully calibrated, we find that the Rabi frequency scales linearly with drive amplitude, even when all three spins are driven simultaneously. We also determine that heating-induced resonance frequency shifts from off-resonant drives are comparable to typical temporal drifts. Our results indicate that the previously observed nonlinear response is not a general feature of LD spin qubits.

</details>
<br>

**关键词**: electric dipole spin resonance, quantum processor, spin qubits, microwave drive, Rabi frequency, resonant drives, Loss-DiVincenzo qubits, three-qubit system

---

### 18. ❌ Space-sharing and Singleton Bounds for Entanglement-assisted Classical Coding

**作者**: Yuhang Yao, Tushita Prasad, Markus Grassl, Syed Jafar, Hua Sun
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08563v1](http://arxiv.org/abs/2603.08563v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题和摘要明确表明这是一篇关于量子信息理论和编码理论的研究，具体涉及纠缠辅助经典编码（EACC）、量子Singleton界和空间共享论证。论文内容完全围绕量子通信、编码理论和信息论展开，与医学图像分析、医学成像或深度学习医学成像没有任何关联。所有关键词均与论文主题无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了纠缠辅助经典编码（EACC）中的量子Singleton界，通过空间共享论证证明了其紧致性，并针对编码器仅允许局部量子操作的情况建立了新的紧致熵Singleton界。

<details open>
<summary>摘要翻译</summary>

> 近期研究指出，空间共享论证证明了熵量子Singleton界的紧性，该问题在文献中对于仅涉及量子消息、仅涉及经典消息或同时涉及经典与量子消息的多种设定下均未解决。聚焦于纠缠辅助经典编码（EACC）的设定，本文首先详细阐述了该设定下的空间共享论证与紧Singleton界，随后针对纠缠辅助分布于部分编码器且各编码器仅允许进行局域量子操作的情形，为EACC码建立了一个新的紧熵Singleton界。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Recent work has noted that a space-sharing argument proves the tightness of the entropic quantum Singleton bounds, which was left open in the literature for various settings involving only-quantum messages, only-classical messages, or both classical and quantum messages. Focusing on the setting of entanglement-assisted classical coding (EACC), in this letter we first elaborate upon the space-sharing argument and the tight Singleton bound for this setting, and then establish a new tight entropic Singleton bound for EACC codes with entanglement assistance distributed across a subset of encoders when only local quantum operations are allowed at each encoder.

</details>
<br>

**关键词**: entanglement-assisted classical coding, quantum Singleton bound, space-sharing argument, entropic bound, EACC codes, local quantum operations, encoder, classical messages

---

### 19. ❌ Extrapolative Quantum Error Mitigation in Continuous-Variable Systems beyond the Training Horizon

**作者**: Jingpeng Zhang, Shengyong Li, Jie Han, Qianchuan Zhao, Jing Zhang, Zeliang Xiang
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08548v1](http://arxiv.org/abs/2603.08548v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究量子计算中的连续变量系统误差缓解，使用基于Swin Transformer的机器学习方法处理量子噪声，与医学图像分析、医学成像或深度学习医学成像完全无关。论文主题是量子信息处理，而非医学领域。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文提出了一种基于时间条件Swin Transformer的外推量子误差缓解框架，用于在连续变量量子系统中抑制噪声，实现了在长时间演化下的准确状态恢复，无需大量训练数据。

<details open>
<summary>摘要翻译</summary>

> 连续变量量子系统为量子信息处理提供了一个通用平台，其中量子态可在正交相空间中表示。在实际实现中，环境噪声（主要是光子损耗和退相）会逐步使这些态退化。基于机器学习的量子误差缓解技术近期已成为抑制此类噪声的一种前景广阔的方法；然而，现有方法通常受限于训练范围，且需要覆盖整个演化过程的训练数据，这在实验上要求苛刻。本文提出了一种基于时间条件化Swin Transformer的外推式量子误差缓解框架。通过自适应层归一化显式嵌入演化时间，该模型学习到一个校正映射，该映射既能解释噪声的连续累积，又能捕捉非局域相空间关联。在马尔可夫与非马尔可夫噪声下的数值模拟表明，该方法能在长时间演化区域实现精确的态恢复，而现有方法在此区域性能会显著下降。我们的研究结果确立了外推式量子误差缓解作为一条实用路径，可在无需海量训练数据的情况下缓解连续变量量子系统中的噪声。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Continuous-variable (CV) quantum systems provide a versatile platform for quantum information processing, in which quantum states can be represented in the quadrature phase space. In realistic implementations, environmental noise, primarily photon loss and dephasing, progressively degrades these states. Machine-learning-based quantum error mitigation (QEM) has recently emerged as a promising approach to suppress such noise; however, existing methods are typically limited to the training horizon and require training data that cover the entire evolution, which is experimentally demanding. Here we introduce a framework for extrapolative quantum error mitigation based on a time-conditioned Swin Transformer. By explicitly embedding the evolution time via adaptive layer normalization, the model learns a correction map that accounts for the continuous accumulation of noise while capturing nonlocal phase-space correlations. Numerical simulations under both Markovian and non-Markovian noise demonstrate accurate state recovery in the long-time regime, where existing approaches deteriorate. Our results establish extrapolative QEM as a practical route to mitigating noise in CV quantum systems without exhaustive training data.

</details>
<br>

**关键词**: quantum error mitigation, continuous-variable quantum systems, Swin Transformer, extrapolative QEM, time-conditioned model, noise suppression, state recovery, machine learning

---

### 20. ❌ The robustness of composite pulses elucidated by classical mechanics. II. The role of initial state imperfection

**作者**: Jonathan Berkheim, David J. Tannor
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08522v1](http://arxiv.org/abs/2603.08522v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究的是核磁共振（NMR）中复合脉冲（Composite Pulses）的鲁棒性分析，特别是针对初始状态不完美的情况。研究使用经典力学框架和布洛赫球面分析脉冲序列性能。所有评分关键词（medical image analysis, medical imaging, deep learning medical imaging）均涉及医学影像分析领域，但本文完全不涉及医学影像的获取、处理、分析或诊断应用，也不涉及深度学习技术。论文属于物理学、量子控制和核磁共振方法学领域，与医学影像分析无直接关联。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该研究通过经典力学框架分析了核磁共振中复合脉冲在初始状态存在分布时的鲁棒性，发现Levitt脉冲序列在考虑初始条件分布时仍保持较好鲁棒性，并通过数值优化找到了性能更优的变体序列。

<details open>
<summary>摘要翻译</summary>

> 在核磁共振（NMR）中，复合脉冲（Composite Pulses, CPs）被广泛用于校正脉冲缺陷，例如射频场不均匀性和共振偏移。尽管多年来已发展出多种鲁棒的脉冲序列，但初始态的不完美作为一种额外的系统误差，在文献中尚未得到广泛讨论。在先前的工作中，我们建立了一个经典正则框架来进行稳定性分析，并将其作为衡量复合脉冲鲁棒性的标准。该工作允许单一初始条件在各种脉冲缺陷下演化。当前研究将此方法扩展至布洛赫球面上初始条件的二维分布；目标是在最大化整个分布布居数反转的同时，最小化分布面积以保持相干性。作为案例研究，我们考察了Levitt提出的$90(x)180(y)90(x)$脉冲序列在初始条件存在展宽时的表现。正则框架使我们能够评估Levitt脉冲序列的鲁棒性，结果发现即使考虑初始条件的展宽，其鲁棒性在很大程度上仍得以保持。然而，通过数值优化，我们发现了Levitt脉冲序列的几种变体，在初始条件存在展宽时能产生更大的相干布居数反转。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> In nuclear magnetic resonance (NMR), Composite Pulses (CPs) are widely used to correct for pulse imperfections, e.g., RF field inhomogeneity and resonance offset. Although robust pulse sequences have been developed throughout the years, the imperfection of the initial state has not been widely discussed in the literature as an additional systematic error. In previous work, we developed a classical canonical framework to perform stability analysis and used this as a measure of CP robustness. In that work, a single initial condition was allowed to evolve under various pulse imperfections. The current work extends this approach to $2D$ distributions of initial conditions on the Bloch Sphere; the objective is to minimize the area in order to preserve coherence, while maximizing population inversion of the entire distribution. As a case study, we investigate Levitt's $90(x)180(y)90(x)$ pulse sequence, when there is a spread in initial conditions. The canonical framework enables us to assess the robustness of Levitt's pulse sequence, and we find that it is maintained to a great extent even when considering a spread of initial conditions. Nevertheless, by conducting a numerical optimization, we have identified several variants of Levitt's pulse sequence that produce a larger coherent population inversion when there is a spread in initial conditions.

</details>
<br>

**关键词**: composite pulses, nuclear magnetic resonance, robustness analysis, Bloch sphere, initial state imperfection, pulse sequence optimization, Levitt sequence, coherence preservation

---

### 21. ❌ Mid-Infrared Modulation of Quantum Emitters in Hexagonal Boron Nitride

**作者**: Karin Yamamura, Xinyang Yu, Chaohao Chen, Mehran Kianinia, Christophe Galland, Igor Aharonovich
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08466v1](http://arxiv.org/abs/2603.08466v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究的是量子发射器在六方氮化硼中的中红外调制，属于量子技术、材料科学和纳米光子学领域。论文内容完全不涉及医学图像分析、医学成像或深度学习医学成像。摘要和标题中没有任何与医学图像相关的术语或概念。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了如何利用中红外激发调制六方氮化硼中单光子发射器的发射特性，提出了一种可逆、非破坏性的方法来增强蓝色单光子发射器的发射。

<details open>
<summary>摘要翻译</summary>

> 单光子发射器（SPEs）是量子技术实用器件中极具前景的基础单元。传统上，这些系统通过声子跃迁使用非共振可见光进行激发，但这一过程至今尚未得到充分理解。本文研究了中红外（MIR）激发对六方氮化硼中单光子发射器特性的相互作用。值得注意的是，我们提出了一种可逆、非破坏性的方法，利用中红外协同激发来增强蓝色单光子发射器的发光。通过共振驱动缺陷局域化的面内红外活性光学声子模式（波长约7.3微米附近），中红外场通过声子辅助复合过程调控载流子动力学。这一在固体缺陷中前所未见的独特现象，为在室温下调控量子发射器提供了日益丰富的工具箱中一个极具潜力的调控手段，有望推动其在实用量子技术中的应用。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Single photon emitters (SPEs) are promising building blocks for practical devices in quantum technologies. Traditionally, these systems are excited using off-resonant visible light through their phonon transitions, yet this process remains poorly understood. Here, we explore the interaction of mid-infrared (MIR) excitation on the properties of SPEs in hexagonal boron nitride. Notably, we present a reversible, non-destructive method to enhance emission from blue SPEs using MIR co-excitation. By resonantly driving defect-localized in-plane infrared-active optical phonon modes near 7.3 um, the MIR field modulates carrier dynamics through a phonon-assisted recombination. This unique feature, not observed previously for defects in solids, is a promising reservoir in a growing toolkit to modulate quantum emitters at room temperature for their use in practical quantum technologies.

</details>
<br>

**关键词**: quantum emitters, hexagonal boron nitride, mid-infrared excitation, single photon emitters, phonon-assisted recombination, room temperature modulation, quantum technologies

---

### 22. ❌ The Transfer Tensor Method: an Analytical Study Case

**作者**: Marcel Morillas-Rozas, Alberto López-García, Gonzalo Reina Rivero, Jianshu Cao, Javier Cerrillo
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08458v1](http://arxiv.org/abs/2603.08458v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文研究的是量子物理中的转移张量方法，用于分析开放量子系统的非马尔可夫动力学，具体案例是二能级原子与损耗腔的相互作用。全文内容涉及量子力学、张量方法、非马尔可夫性等物理概念，未提及任何医学图像分析、医学成像或深度学习在医学成像中的应用。因此，所有关键词与论文内容完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了转移张量方法在开放量子系统非马尔可夫动力学分析中的应用，通过解析可解模型展示了该方法与Nakajima-Zwanzig方程内存核的差异，并识别了增强非马尔可夫性的参数区域。

<details open>
<summary>摘要翻译</summary>

> 转移张量方法是一种用于分析和传播一般开放量子系统的多功能工具。它通过对一组动力学映射进行直接变换，以紧凑的方式捕捉非马尔可夫系统中的所有记忆效应。转移张量提供了与开放量子系统过去演化过程中给定时间离散化相关联的精确卷积传播子。本文证明，对于任何有限时间离散化，Nakajima-Zwanzig 方程的记忆核会偏离精确的转移张量，尽管两者如预期那样在连续时间极限下收敛。我们在一个解析可解模型的背景下检验了这一行为：即 Jaynes-Cummings 极限下与损耗腔共振的两能级原子。原子动力学分解为两个解耦的自由度——相干性和布居数反转。我们推导了支配相干性的动力学映射、转移张量和记忆核的精确表达式，并将其与布居数反转的对应量联系起来。作为腔损耗率与原子-腔耦合强度之比的函数，我们识别出了非马尔可夫性增强的区域，在这些区域中，对于特定的时间步长选择，系统可以被描述为完全马尔可夫的。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> The transfer tensor method is a versatile tool for analyzing and propagating general open quantum systems. It captures in a compact manner all memory effects in a non-Markovian system through a straightforward transformation of a set of dynamical maps. Transfer tensors provide the exact convolutional propagator associated with a given time discretization over the past evolution of an open quantum system. Here we show that, for any finite time discretization, the memory kernel of the Nakajima Zwanzig equation deviates from the exact transfer tensors, although both converge in the continuous-time limit, as expected. We examine this behaviour in the context of an analytically solvable model: a two level atom resonant with a lossy cavity in the Jaynes Cummings limit. The atomic dynamics separate into two decoupled degrees of freedom -- the coherence and the population inversion. We derive exact expressions for the dynamical map, the transfer tensors and the memory kernel governing the coherence, and we relate them to their counterparts for the population inversion. As a function of the ratio between the cavity loss rate and the atom-cavity coupling strength, we identify regions of enhanced non-Markovianity in which the system can be described as fully Markovian for certain time-step choices.

</details>
<br>

**关键词**: transfer tensor method, open quantum systems, non-Markovian dynamics, Nakajima-Zwanzig equation, two-level atom, lossy cavity, Jaynes-Cummings model, memory kernel

---

### 23. ❌ Stochastic Loop Corrections to Belief Propagation for Tensor Network Contraction

**作者**: Gi Beom Sim, Tae Hyeon Park, Kwang S. Kim, Yanmei Zang, Xiaorong Zou, Hye Jung Kim, D. ChangMo Yang, Soohaeng Yoo Willow, Chang Woo Myung
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08427v1](http://arxiv.org/abs/2603.08427v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为《Stochastic Loop Corrections to Belief Propagation for Tensor Network Contraction》，摘要内容聚焦于张量网络收缩、置信传播算法、环路校正、马尔可夫随机场、伊辛模型、蒙特卡洛采样等计算物理和统计力学领域的概念。全文未提及任何医学图像分析、医学成像或深度学习在医学成像中的应用。论文的研究背景虽然是'Medical image analysis'，但实际论文内容与此背景完全不符，因此所有关键词的相关度均为0分。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文提出了一种结合置信传播和随机环路校正的混合方法，用于精确计算具有对称边势的成对马尔可夫随机场的配分函数，并以二维铁磁伊辛模型为例验证了其有效性。

<details open>
<summary>摘要翻译</summary>

> 张量网络缩并是量子多体物理、统计力学和机器学习领域的基础性计算难题。置信传播（Belief Propagation, BP）提供了一种高效的近似解法，但在含环图结构上会引入系统性误差。本文提出一种混合方法，通过对BP的环修正项进行随机采样来获得精确结果，并以二维铁磁性伊辛模型为例展示了该方法的有效性。对于任何具有对称边势的成对马尔可夫随机场，我们的方法将配分函数精确分解为BP贡献项与环修正因子的乘积，其中环修正因子对所有有效环构型求和，其权重由势函数直接导出的边权重决定。我们采用马尔可夫链蒙特卡洛方法对该求和进行采样，其中采样转移操作始终保持环约束条件，并结合伞形采样技术以确保在所有关联强度区间的高效探索。这种随机方法能够在任意参数区间内提供具有可控统计误差的无偏估计。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Tensor network contraction is a fundamental computational challenge underlying quantum many-body physics, statistical mechanics, and machine learning. Belief propagation (BP) provides an efficient approximate solution, but introduces systematic errors on graphs with loops. Here, we introduce a hybrid method that achieves exact results by stochastically sampling loop corrections to BP and showcase our method by applying it to the two-dimensional ferromagnetic Ising model. For any pairwise Markov random field with symmetric edge potentials, our approach exploits an exact factorization of the partition function into the BP contribution and a loop correction factor summing over all valid loop configurations, weighted by edge weights derived directly from the potentials. We sample this sum using Markov chain Monte Carlo with moves that preserve the loop constraint, combined with umbrella sampling to ensure efficient exploration across all correlation strengths. Our stochastic approach provides unbiased estimates with controllable statistical error in any parameter regime.

</details>
<br>

**关键词**: tensor network contraction, belief propagation, loop corrections, Markov random field, Ising model, Monte Carlo sampling, partition function

---

### 24. ❌ Experimental Realization of the Markov Chain Monte Carlo Algorithm on a Quantum Computer

**作者**: Baptiste Claudon, Sergi Ramos-Calderer, Jean-Philip Piquemal
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08395v1](http://arxiv.org/abs/2603.08395v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为《Experimental Realization of the Markov Chain Monte Carlo Algorithm on a Quantum Computer》，摘要内容完全围绕量子计算、量子算法（如Quantum Amplitude Estimation）、量子马尔可夫链蒙特卡洛算法（qMCMC）在量子计算机上的实验实现展开。论文未提及任何医学图像分析、医学成像或深度学习在医学成像中的应用，研究领域属于量子计算与算法实现，与给定的医学图像分析关键词完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了在量子计算机上实验实现量子马尔可夫链蒙特卡洛算法，并在当前NISQ硬件上获得了准确结果。

<details open>
<summary>摘要翻译</summary>

> 量子算法在特定采样任务上相较经典算法呈现二次复杂度优势。例如，量子振幅估计算法（Quantum Amplitude Estimation, QAE）能够在获取对应待采样概率分布的量子态时，加速特定函数均值的估计过程。经典方法通常通过运行马尔可夫链步骤获取样本。本工作中，我们通过实验利用马尔可夫链的编码制备量子态，并在Quantinuum公司的H2与Helios量子计算机上运行量子马尔可夫链蒙特卡洛算法（quantum Markov Chain Monte Carlo, qMCMC）。实验结果表明，直接在物理量子比特上操作的当前含噪声中等规模量子（Noisy Intermediate Scale Quantum, NISQ）硬件能够获得精确计算结果。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Quantum algorithms present a quadratically improved complexity over classical ones for certain sampling tasks. For instance, the Quantum Amplitude Estimation (QAE) algorithm promises to speedup the estimation of the mean of certain functions, given access to the quantum state corresponding to the probability distribution to be sampled from. Classically, samples are often obtained by running steps a Markov Chain. In this work, we experimentally use encodings of Markov chains to prepare quantum states and run a quantum Markov Chain Monte Carlo algorithm (qMCMC) on Quantinuum's H2 and Helios quantum computers. We demonstrate that it is possible to obtain accurate results on current Noisy Intermediate Scale Quantum (NISQ) hardware, operating directly on the physical qubits.

</details>
<br>

**关键词**: quantum computer, Markov Chain Monte Carlo, quantum algorithm, Quantum Amplitude Estimation, qMCMC, NISQ hardware, quantum state preparation

---

### 25. ❌ Bound states in a semi-infinite square potential well

**作者**: Nivaldo A. Lemos
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08394v1](http://arxiv.org/abs/2603.08394v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为《Bound states in a semi-infinite square potential well》，摘要讨论的是量子力学中半无限方势阱的粒子能级、本征函数和概率计算，属于理论物理领域。所有评分关键词（medical image analysis, medical imaging, deep learning medical imaging）均涉及医学图像分析，与论文的量子力学主题完全无关，因此相关度均为0分。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了量子力学中半无限方势阱的粒子能级问题，通过图形法和简化方程找到了高精度近似解，并构建了归一化本征函数及粒子在阱内的概率。

<details open>
<summary>摘要翻译</summary>

> 有限方形势阱是量子力学导论中的经典问题。关于其允许能级的确定已有大量文献，这通常需要通过数值、图解法或近似解析方法求解超越方程。本文研究的是较少被探讨的半无限势阱中的粒子问题。通过标准图解法求解同样由超越方程决定的能量本征值，并给出了确定定态数量的简明规则。随后尝试对上述超越方程进行简化。在此过程中，我们发现了若干陷阱，并指出某优秀教材习题解答手册中提出的、声称更简化的图解法存在缺陷。通过更细致的分析，我们提出了正确的简化方法，该方法特别适用于求解高精度的能级近似值。最后，我们构建出一类精确解，给出相应的归一化本征函数，并计算了粒子在势阱内的发现概率。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> The finite square potential well is a staple problem in introductory quantum mechanics. There is an extensive literature on the determination of the allowed energies, which requires the solution of a transcendental equation by numerical, graphical or approximate analytic methods. Here we investigate the less explored problem of a particle in a semi-infinite potential well. The energy eigenvalues, which are also determined by a transcendental equation, are found by a standard graphical method, and a simple rule that yields the number of stationary states is provided. Next a simplification of the aforementioned transcendental equation is attempted. During the process pitfalls are encountered and a purportedly simpler graphical treatment of the problem given in the solutions manual to a fine textbook is shown to be flawed. A more careful analysis brings forth the correct simplification, which is shown to be particularly suitable for finding highly accurate approximations to the energy levels. Finally, a class of exact solutions is produced, the associated normalized eigenfunctions are constructed and the probability of finding the particle inside the well is computed.

</details>
<br>

**关键词**: semi-infinite potential well, energy eigenvalues, transcendental equation, graphical method, normalized eigenfunctions, quantum mechanics, square potential well, bound states

---

### 26. ❌ Dynamical Lie algebras generated by Pauli strings and quadratic spaces over $\mathbb{F}_2$

**作者**: Hans Cuypers
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08373v1](http://arxiv.org/abs/2603.08373v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题和摘要明确表明这是一篇纯数学论文，研究动态李代数（Dynamical Lie algebras）和Pauli字符串的数学结构，属于量子计算和代数领域。文中未提及任何医学图像分析、医学成像或深度学习在医学成像中的应用。所有关键词与论文内容完全无关。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该论文研究了由Pauli字符串生成的动态李代数的数学结构，并提出了一种算法来确定其同构类型。

<details open>
<summary>摘要翻译</summary>

> 动力学李代数，即由泡利串生成的 $\mathfrak{su}(2^n)$ 的李子代数，近来受到深入研究。它们也被称为泡利李代数或哈密顿李代数。
> 本文针对泡利李代数的各类近期成果，提出了一种统一的数学处理方法。此外，我们提出一种算法，当输入一组泡利串时，该算法可在 $\mathcal{O}(\max(n,m)^3)$ 时间内确定由这些泡利算符生成的动力学李代数的同构类型，其中 $m$ 是生成集的大小。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Dynamical Lie algebras, i.e. Lie subalgebras of $\mathfrak{su}(2^n)$, generated by Pauli strings have recently been studied intensively. They are also called Pauli Lie algebras or Hamiltonian Lie algebras.
>   In this paper we provide a uniform mathematical approach to various recent results on Pauli Lie algebras. Moreover, we present an algorithm that on input of a set of Pauli strings determines the isomorphism type of the dynamical Lie algebra generated by these Pauli's in time $\mathcal{O}(\max(n,m)^3)$ where $m$ is the size of the generating set.

</details>
<br>

**关键词**: Dynamical Lie algebras, Pauli strings, Hamiltonian Lie algebras, Lie subalgebras, isomorphism type, algorithm, quadratic spaces, F2

---

### 27. ❌ Bound Trions in Two-Dimensional Monolayers: A Review

**作者**: Roman Ya. Kezerashvili
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08346v1](http://arxiv.org/abs/2603.08346v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为《Bound Trions in Two-Dimensional Monolayers: A Review》，摘要聚焦于二维半导体中的三粒子激发态（trions）的理论与实验研究，涉及量子约束、介电环境、计算方法等物理概念。所有评分关键词（medical image analysis, medical imaging, deep learning medical imaging）均与医学影像分析或深度学习在医学影像中的应用相关，而论文内容完全围绕凝聚态物理中的二维材料与准粒子物理，未提及任何医学、影像或深度学习内容。因此，所有关键词相关度均为0分，加权总分为0.0分，远低于动态及格分14.0分。作者列表中未包含指定专家。

</details>
<br>

!!! tip deepseek-chat TL;DR

    这篇综述论文研究了二维单层材料中三粒子激发态（trions）的物理性质，总结了其在理论计算和实验观测方面的进展，并分析了介电环境、外场效应及维度降低对其稳定性和结构的影响。

<details open>
<summary>摘要翻译</summary>

> 激子——由两个同种电荷载流子与一个异种电荷载流子通过库仑力束缚形成的三粒子激发态——是二维半导体中的核心准粒子。减弱的介电屏蔽效应和量子限域效应显著增强了其结合能，使其具有强稳定性并易于实验观测。本综述系统梳理了激子物理领域的理论与实验进展，重点阐述了严格的多体处理方法，以及介电环境、各向异性、外加电场和磁场的影响。我们分析了在二维位形空间中描述激子的计算方法，并讨论了维度降低如何改变其结构与稳定性。文章亦强调了其与多体现象的关联，包括屏蔽效应、朗道能级混合以及激子-极化子交叉现象。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Trions -- Coulomb-bound three-particle excitations composed of two like-charge carriers and one oppositely charged carrier -- are central quasiparticles in two-dimensional semiconductors. Reduced dielectric screening and quantum confinement strongly enhance their binding energies, making them robust and experimentally accessible. This review surveys theoretical and experimental advances in trion physics, emphasizing rigorous few-body approaches and the role of dielectric environment, anisotropy, and external electric and magnetic fields. We analyze computational methods for describing trions in two-dimensional configuration spaces and discuss how reduced dimensionality modifies their structure and stability. Connections to many-body phenomena, including screening, Landau-level mixing, and exciton--polaron crossover, are also highlighted.

</details>
<br>

**关键词**: trions, two-dimensional semiconductors, quantum confinement, dielectric environment, few-body approaches, exciton-polaron crossover, binding energies, computational methods

---

### 28. ❌ A Realistic Framework for Quantum Sensing under Finite Resources

**作者**: Zdeněk Hradil, Jaroslav Řeháček
**期刊/来源**: arxiv
**发布日期**: 2026-03-09
**arXiv链接**: [http://arxiv.org/abs/2603.08306v1](http://arxiv.org/abs/2603.08306v1)

**评分**: 0.0 / 14.0 ❌

<details>
<summary>评分详情</summary>

| 关键词 | 权重 | 相关度 | 得分 |
|------|------|------|------|
| medical image analysis | 1.0 | 0.0/10 | 0.0 |
| medical imaging | 1.0 | 0.0/10 | 0.0 |
| deep learning medical imaging | 1.0 | 0.0/10 | 0.0 |

**评分理由**: 论文标题为《A Realistic Framework for Quantum Sensing under Finite Resources》，摘要内容完全围绕量子传感、量子费舍尔信息、贝叶斯框架、相位估计、NOON态、Holland-Burnett干涉仪、压缩态等量子物理和计量学概念展开。全文未提及任何医学图像分析、医学成像或深度学习在医学成像中的应用，与给定的三个关键词无任何关联。

</details>
<br>

!!! tip deepseek-chat TL;DR

    该研究建立了一个在有限资源下评估量子传感性能的现实端到端框架，结果表明某些非经典方案（如NOON态）在有限先验宽度下相对于经典干涉测量并无性能优势，并明确了量子费舍尔信息作为可靠诊断工具的条件。

<details open>
<summary>摘要翻译</summary>

> 量子增强传感通常以量子费舍尔信息（QFI）作为基准指标，该量常被直接解读为可达精度的指示器。然而，这一量仅在一个完整设定的推断框架内才具有操作意义，该框架需一致地包含态制备、测量设计、资源核算、估计量构建、先验信息以及有限数据效应。本文建立了一个有限资源下量子传感的现实端到端框架，并明确了获得具有操作意义的性能评估所需的一般原则。一个核心概念要点是：估计的相关单元并非单个探测事件，而是构建一致估计量所需的推断数据集。我们将此方法应用于文献中常引用的几种范式传感策略。在贝叶斯框架下，以相等总光子资源重新审视使用NOON态的相位估计，我们显式构建了最优估计量，并证明在有限先验宽度下进行全局相位估计时，此类方案相比重复的经典干涉测量并无性能优势。表面上的海森堡级标度主要源于先验约束，而非测量中获得的信息——在此处所考虑的归一化资源意义上，其操作意义可忽略。我们进一步分析了霍兰德-伯内特干涉测量与使用压缩态的外差探测，阐明了估计量构建和重复次数如何决定可达精度，以及QFI何时能提供可靠的诊断依据。我们的研究结果明确了非经典资源在何种条件下能带来真正的计量学优势，并为在实际实验约束下设计和评估量子传感方案提供了实用方法论。

</details>
<br>

<details>
<summary>摘要 (Abstract)</summary>

> Quantum-enhanced sensing is commonly benchmarked using the quantum Fisher information (QFI), often interpreted as a direct indicator of achievable precision. However, this quantity acquires operational meaning only within a fully specified inference framework that consistently incorporates state preparation, measurement design, resource accounting, estimator construction, prior information, and finite data effects. Here we establish a realistic end-to-end framework for quantum sensing under finite resources and identify general principles required for operationally meaningful performance assessment. A central conceptual point is that the relevant unit of estimation is not a single detection event but the inference data set required to construct a consistent estimator. We apply this approach to several paradigmatic sensing strategies frequently cited in the literature. Revisiting phase estimation with NOON states within a Bayesian framework under equal total photon resources, we explicitly construct optimal estimators and show that such schemes offer no performance advantage over repeated classical interferometry for global phase estimation with finite prior width. The apparent Heisenberg-like scaling arises predominantly from prior constraints rather than from information gained in the measurement, which is operationally negligible in the resource-normalized sense considered here. We further analyse Holland-Burnett interferometry and homodyne detection with squeezed states, demonstrating how estimator construction and repetition number determine the attainable precision and when QFI provides a reliable diagnostic. Our results clarify the conditions under which nonclassical resources lead to genuine metrological advantages and provide a practical methodology for designing and evaluating quantum sensing protocols under realistic experimental constraints.

</details>
<br>

**关键词**: quantum sensing, quantum Fisher information, finite resources, phase estimation, NOON states, Bayesian framework, metrological advantages, estimator construction

---
